...
struct vgic_dist_map {
    ...
    uint32_t irq_group0[CONFIG_MAX_NUM_NODES];        /* [0x0080, 0x0084) */
    uint32_t irq_group[31];        /* [0x0084, 0x0100) */

    uint32_t enable_set0[CONFIG_MAX_NUM_NODES];        /* [0x0100, 0x0104) */
    uint32_t enable_set[31];        /* [0x0104, 0x0180) */
    uint32_t enable_clr0[CONFIG_MAX_NUM_NODES];        /* [0x0180, 0x0184) */
    uint32_t enable_clr[31];        /* [0x0184, 0x0200) */
    uint32_t pending_set0[CONFIG_MAX_NUM_NODES];        /* [0x0200, 0x0204) */
    uint32_t pending_set[31];       /* [0x0204, 0x0280) */
    uint32_t pending_clr0[CONFIG_MAX_NUM_NODES];       /* [0x0204, 0x0280) */
    uint32_t pending_clr[31];       /* [0x0280, 0x0300) */
    uint32_t active_set0[CONFIG_MAX_NUM_NODES];        /* [0x0300, 0x0304) */
    uint32_t active_set[31];        /* [0x0304, 0x0380) */
    uint32_t active_clr0[CONFIG_MAX_NUM_NODES];        /* [0x0380, 0x0384) */
    uint32_t active_clr[31];        /* [0x0384, 0x0400) */

    uint32_t priority0[CONFIG_MAX_NUM_NODES][8];         /* [0x0400, 0x0420) */
    uint32_t priority[247];         /* [0x0420, 0x07FC) */
    uint32_t res6;                  /* 0x07FC */

    uint32_t targets0[CONFIG_MAX_NUM_NODES][8];         /* [0x0800, 0x0820) */
    uint32_t targets[247];          /* [0x0820, 0x0BFC) */
    uint32_t res7;                  /* 0x0BFC */

    uint32_t config[64];            /* [0x0C00, 0x0D00) */
    uint32_t group_mod[64];         /* [0x0D00, 0x0E00) */
    uint32_t nsacr0[CONFIG_MAX_NUM_NODES]; /* [0x0E00, 0x0E04) */
    uint32_t nsacr[63];             /* [0x0E04, 0x0F00) */
    uint32_t sgir;                  /* 0x0F00 */
    uint32_t res8[3];               /* [0x0F04, 0x0F10) */
    uint32_t sgi_pending_clr[CONFIG_MAX_NUM_NODES][4];    /* [0x0F10, 0x0F20) */
    uint32_t sgi_pending_set[CONFIG_MAX_NUM_NODES][4];    /* [0x0F20, 0x0F30) */
...
struct vgic_rdist_sgi_map {  /* Starting */
...
typedef struct {
    ...
    struct vgic_dist_map dist;
    /// Virtual redistributer registers for control and physical LPIs
    struct vgic_rdist_map rdist[CONFIG_MAX_NUM_NODES];
    /// Virtual redistributer for SGI and PPIs
    struct vgic_rdist_sgi_map rdist_sgi[CONFIG_MAX_NUM_NODES];
} vgic_reg_t;

static inline bool gic_dist_is_enabled(struct vgic_dist_map *gic_dist)
{
    return gic_dist->ctlr & GIC_500_GRP1_NS;
}

static inline int gic_dist_enable(struct vgic_dist_map *gic_dist)
{
    gic_dist->ctlr |= GIC_500_GRP1_NS | GIC_500_ARE_S;
}

static inline int gic_dist_disable(struct vgic_dist_map *gic_dist)
{
    gic_dist->ctlr &= ~(GIC_500_GRP1_NS | GIC_500_ARE_S);
}

static inline struct vgic_dist_map *priv_get_dist(void *priv)
{
    assert(priv);
    return &((vgic_reg_t *) priv)->dist;
}

extern const struct vgic_device dev_vgic_rdist;
extern const struct vgic_device dev_vgic_rdist_sgi;
