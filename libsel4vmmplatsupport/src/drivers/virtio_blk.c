                                              ...
                                              unsigned int interrupt_line)
{
    ...
    int err = ps_calloc(&ops.malloc_ops, 1, sizeof(*pci_config),
                        (void **)&pci_config);
    ZF_LOGF_IF(err, "Failed to allocate pci config");
    *pci_config = (vmm_pci_device_def_t) {
        .vendor_id = VIRTIO_PCI_VENDOR_ID,
        .device_id = VIRTIO_BLK_PCI_DEVICE_ID,
        .command = PCI_COMMAND_IO | PCI_COMMAND_MEMORY,
        .header_type = PCI_HEADER_TYPE_NORMAL,
        .subsystem_vendor_id    = VIRTIO_PCI_SUBSYSTEM_VENDOR_ID,
        .subsystem_id       = VIRTIO_ID_BLK,
        .interrupt_pin = interrupt_pin,
        .interrupt_line = interrupt_line,
        .bar0 = iobase | PCI_BASE_ADDRESS_SPACE_IO,
        .cache_line_size = 64,
        .latency_timer = 64,
        .prog_if = VIRTIO_PCI_CLASS_BLK & 0xff,
        .subclass = (VIRTIO_PCI_CLASS_BLK >> 8) & 0xff,
        .class_code = (VIRTIO_PCI_CLASS_BLK >> 16) & 0xff,
    };
                                     ...
                                     struct raw_iface_funcs backend)
{
    ...
    vmm_pci_entry_t entry = vmm_virtio_blk_pci_bar(io_entry->range.start, iobase_size_bits, interrupt_pin, interrupt_line);
